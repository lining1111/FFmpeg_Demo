...
/*************************************************
	...
	Return:			true : 命令行解析正确
					false : 命令行解析错误
*************************************************/
static int read_yuv_from_ifile(uint8_t *src_data[4], int src_linesize[4], int srcWidth, int srcHeight, int color_plane,
                               IOFiles &files) {
    ...
    if (frame_width == frame_stride) {
        //宽度和跨度相等，像素信息连续存放
        fread(src_data[color_plane], 1, frame_size, files.iFile);
    } else {
        //宽度小于跨度，像素信息保存空间之间存在间隔
        for (int row_idx = 0; row_idx < frame_height; row_idx++) {
            fread(src_data[color_plane] + row_idx * frame_stride, 1, frame_width, files.iFile);
        }
    }
...
int main(int argc, char **argv) {
    ...
    struct SwsContext *sws_ctx = sws_getContext(srcWidth, srcHeight, src_pix_fmt, dstWidth, dstHeight, dst_pix_fmt,
